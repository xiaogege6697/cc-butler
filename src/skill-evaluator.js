 ...
 *
 * 参考 darwin-skill v2.0 + SkillLens 论文，简化为可独立运行。
 */
...
/**
 * @param {Object} skillStore - Skill 存储接口，需提供：
 *   - get(id): 获取单个 skill
 *   - getAll(filter?): 获取所有/过滤后的 skill 列表
 *   - update(id, patch): 更新 skill
 *   - add(skill): 添加 skill
...
function createSkillEvaluator(skillStore, bus) {
  ...
  async function evaluate(skillId) {
    ...
    // 6. 计算加权总分（归一化到 100）
    const normalizedScore = Math.min(Math.round((totalScore / TOTAL_WEIGHT) * 100), 100);
    ...
    bus.emit('skill.evaluated', { id: skillId, score: normalizedScore });
  ...
  async function evolve(skillIds) {
    ...
    const dimResults = [
      evaluateFrontmatter(tempSkill, evolvedContent),
      evaluateWorkflow(tempSkill, evolvedContent),
      evaluateFailureModes(tempSkill, evolvedContent),
      evaluateCheckpoints(tempSkill, evolvedContent),
      evaluateSpecificity(tempSkill, evolvedContent),
      evaluateResourceIntegration(tempSkill, evolvedContent),
      evaluateArchitecture(tempSkill, evolvedContent),
      evaluateTestPerformance(tempSkill, evolvedContent),
      evaluateAntiPatterns(tempSkill, evolvedContent),
    ];
    ...
    for (let i = 0; i < DIMENSIONS.length; i++) {
      const dim = DIMENSIONS[i];
      newBreakdown[dim.key] = {
        score: dimResults[i].score,
        max: dimResults[i].max,
        weight: dim.weight,
        label: dim.label,
        checks: dimResults[i].checks || {},
      };
      newRawTotal += dimResults[i].score;
    }
    const newScore = Math.min(Math.round((newRawTotal / TOTAL_WEIGHT) * 100), 100);
  ...
  function install(skillId) {
    ...
    bus.emit('skill.installed', { id: skillId, path: targetPath });
  ...
  return { evaluate, evaluateAll, evolve, install, getHistory };
}
